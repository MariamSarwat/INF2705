...
public:
    Poisson( float r = 1.0, float h = 0.0, float ang = 0.0, float vit = 1.0, float tai = 0.5, double rouge = 0.0, double vert = 0.0, double bleu = 0.0 )
        : rayon(r), hauteur(h), angle(ang), vitesse(vit), taille(tai)
    {
        // créer un poisson graphique
        coulSelection = glm::vec3(rouge,vert,bleu); // attribution de la couleur au poisson
    ...
    void afficher()
    {
        matrModel.PushMatrix();{ // sauvegarder la tranformation courante
            ...
            //si en mode Selection, on utilise la couleur de selection, sinon on utilise seulement la couleur du corps
            if(Etat::enSelection){
                glVertexAttrib3fv( locColor, glm::value_ptr(coulSelection) );
			} else {
            ...
            // afficher les yeux
            // (en utilisant le cylindre centré dans l'axe des Z, de rayon 1, entre (0,0,0) et (0,0,1))
            //si en mode Selection, on utilise la couleur de selection, sinon on utilise seulement la couleur des yeux
            if(Etat::enSelection){
				glVertexAttrib3fv( locColor, glm::value_ptr(coulSelection) );
			} else {
    ...
    //le poisson avance seulement s'il n'est pas selectionné
    ...
    //avoir acces a la couleur de selection
	glm::vec3 getCoulSelection(){
		return coulSelection;
	}

    // avoir acces au taux de rouge de la couleur (on multipli par 255 pour compare avec le pixel selectionne, celui-ci sort en ubyte donc pou comparer il faut faire 8255)
    int getCoulRouge(){
		return coulSelection[0]*255;
	}


    bool estSelectionne = false;        // les variables du poisson
    glm::vec3 coulSelection;            // couleur de sélection du poisson
