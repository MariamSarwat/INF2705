...
public:
    ...
    void initialiserGraphique()
    {
        ...
        glGenBuffers( 1, &vboTheiereSommets );// lier l'objet tampon afin de l'utiliser
        
        glBindBuffer( GL_ARRAY_BUFFER, vboTheiereSommets );// charger le tableau de sommets dans ce VBO
        ...
        glGenBuffers( 1, &vboTheiereConnec );// lier l'objet tampon afin de l'utiliser
        glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, vboTheiereConnec );// charger le tableau de connectivité dans ce VBO
    ...
    void afficherCorps()
    {
        ...
        matrModel.PushMatrix();{
            ...
            matrModel.Scale(taille , taille, 2*taille  );
    ...
    void afficherAiles()
    {
        ...
        matrModel.PushMatrix();{
            matrModel.Translate(0.0, taille,0);
            matrModel.Rotate(angleAile, 1.0, 0.0, 0.0);
            
            matrModel.Translate( 0.0, -taille, 0);
            matrModel.Translate( 0.0, 2*taille, 0.0 ); // (bidon) À MODIFIER
            matrModel.Scale(2*taille, 2*taille, 0);
    ...
    void afficherBrasJambes()
    {
        ...
        // les bras
        matrModel.PushMatrix();{
            matrModel.Translate( 0, taille+((longMembre/2)* 0.70710678118), -longMembre/2); // (bidon) À MODIFIER
            matrModel.Rotate(45.0, 1.0, 0.0, 0.0);
        
            matrModel.Translate( 0, -taille, +longMembre/2); // (bidon) À MODIFIER
        ...
        matrModel.PushMatrix();{
            ...
            matrModel.Translate( 0, taille, -2*taille); // (bidon) À MODIFIER
            matrModel.Rotate(angleJambe, 0.0, -1.0, 0.0);
            matrModel.Translate( 0, -taille, + 2*taille); // (bidon) À MODIFIER

            matrModel.Translate( 0, taille, - 2*taille - longMembre/2); // (bidon) À MODIFIER
            matrModel.Scale(largMembre, largMembre, longMembre);
